...
/// <summary>
/// 마우스를 따라다니며 화면 내부를 벗어나지 않아야 하는 UI에 부착. 주로 툴팁 UI에 사용.
/// </summary>
public class UI_FollowMouse : MonoBehaviour
{
    ...
    private void SetPosition()
    {
        ...
        // 기본적으로 마우스 오른쪽 밑에 ui를 띄움 (변경 가능)
        Vector2 pivot = new(0, 1);
        Vector2 offset = new(pivot.x == 1 ? -_padding.x : _padding.x, pivot.y == 1 ? _padding.y : -_padding.y);

        // 화면 오른쪽을 벗어나면 마우스 왼쪽에 붙임
        if (anchoredPos.x + rectSize.x + _padding.x > canvasSize.x / 2f)
        {
            pivot.x = 1;
            offset.x = -_padding.x;
        }
        // 화면 왼쪽을 벗어나면 마우스 오른쪽에 붙임
        ...
        // 화면 아래를 벗어나면 마우스 위에 붙임
        //if (anchoredPos.y - rectSize.y - _padding.y < -canvasSize.y / 2f)
        //{
        //    pivot.y = 0;
        //    offset.y = -_padding.y;
        //}
        //// 화면 위를 벗어나면 마우스 아래에 붙임
        //else if (anchoredPos.y + _padding.y > canvasSize.y / 2f)
        //{
        //    pivot.y = 1;
        //    offset.y = _padding.y;
        //}
