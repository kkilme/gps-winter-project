...
public abstract class MoveAttackSkill : BaseSkill
{
    ...
    {   
        ...
        sequence.Append(Owner.transform.DOLookAt(TargetCell.transform.position, 0.3f));

        Vector3 direction = (Owner.transform.position - TargetCell.transform.position).normalized;

        float stopDistance = 1.5f;
        // 상대의 약간 앞까지만 이동
        sequence.Append(
            Owner.transform.DOMove(TargetCell.transform.position + direction * stopDistance, 0.8f)
    ...
    protected virtual Tween Return()
    {
        ...
        sequence.Append(Owner.transform.DOMove(_originalPos, 0.8f)
            .OnStart(() => { _animator.SetBool(GlobalValues.ANIMATION_PARAM_MOVING, true); })
            .OnComplete(() => { _animator.SetBool(GlobalValues.ANIMATION_PARAM_MOVING, false); }));
